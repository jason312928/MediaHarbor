...
- 縮圖會保持橫向或直向媒體的原始比例，並兼容需要來源頁請求標頭的網站
- 支援影片畫質、純音訊及僅字幕下載；可偵測人工與自動字幕、篩選語言、輸出 SRT/VTT/ASS 或 Word RTF 文件，並選擇保留獨立字幕檔或嵌入影片
...
- GitHub Release 的預先編譯套件目前僅支援 Apple Silicon（arm64）；Intel Mac 可使用 Swift 6 從原始碼建構
